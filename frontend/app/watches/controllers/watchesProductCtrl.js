...
['$scope', '$state', '$stateParams', '$sce', '$window', '$interpolate', 'watchesService', 'watchesCategoryService',
	function ($scope, $state, $stateParams, $sce, $window, $interpolate, watchesService, watchesCategoryService) {
	        ...
	        watchesService.getProductById().get({ 'id': $stateParams.id }).$promise.then(
                function (response) {
                    ...
                    if (response.data.length > 0) {
                        ...
                        loadCategoryProducts($scope.productData._id, $scope.productData.title, $scope.productData.brand);
                        $scope.testStr = $interpolate("Hello {{ productData.title }}")($scope);
	    ...
	    var loadCategoryProducts = function (id, title, category) {
	        watchesCategoryService.getMostSelling().get({ 'id': id, 'title': title, 'category': category }).$promise.then(
                function (response) {
                    // success
                    if (response.data.length > 0) {
